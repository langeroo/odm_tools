...
# check arguments
if len(sys.argv) < 2:
	print("Usage: " + sys.argv[0] + " /path/to/DJI_VIDEO_001.MOV (/path/to/DJI_VIDEO_002.MOV) (...)" );
	quit();

mov_files = [];
file_name_no_ext = [];
exif_entries_by_file = [];

print("Will generate JPGs for:")
for i in range(1,len(sys.argv)):
	file = sys.argv[i];
	split_file_name = os.path.splitext(file)
	if split_file_name[1] == ".MOV":
		mov_files.append(file);
		print("\t" + file)

for file_mov in mov_files:
	# get filenames
	# file_mov = sys.argv[1];
	data_file_name = os.path.splitext(os.path.split(file_mov)[1])[0];
	file_name_no_ext.append(data_file_name);
	...
	exif_entries_by_file.append(exif_entries);

	# # run ffmpeg for .mov -> .jpg at 1fps
	print("Extracing JPGs from " + file_mov + "...")
	output_image_string = output_path + "/" + data_file_name + "_%05d.jpg";
	try:
...
all_imgs_by_input_file = [];
for name in file_name_no_ext:
	clip_associated_files = [img for img in all_imgs if name in img];
	clip_associated_files.sort();
	all_imgs_by_input_file.append(clip_associated_files);


for j in range(len(exif_entries_by_file)):
	print("Adding exif data corresponding to video clip: " + file_name_no_ext[j])
	exif_curr_file = exif_entries_by_file[j]
	imgs_curr_file = all_imgs_by_input_file[j]

	i = 0;
	# shove position data from SRT into jpg EXIF data
	# for curr_img in all_imgs:
	for curr_img in imgs_curr_file:
		my_image = None;
		# access exif for an image, set variables as found in SRT
		with open(curr_img, 'rb') as image_file:
			my_image = exif.Image(image_file)
			my_image.datetime 			= exif_curr_file[i][0];
			my_image.gps_latitude 		= exif_curr_file[i][1];
			my_image.gps_latitude_ref 	= exif_curr_file[i][2];
			my_image.gps_longitude 		= exif_curr_file[i][3];
			my_image.gps_longitude_ref 	= exif_curr_file[i][4];
			my_image.gps_altitude 		= exif_curr_file[i][5];
			my_image.gps_altitude_ref 	= exif_curr_file[i][6];
